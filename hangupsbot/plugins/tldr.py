...
def _initialise(bot):
    ...
    if not bot.config.exists(['tldr_echo']):
        bot.config.set_by_path(["tldr_echo"],
                               1)  # TLDR_ECHO_OPTIONS[1] is "GROUP"
        bot.config.save()
...
def tldrecho(bot, event, *dummys):
    ...
    if bot.memory.exists(['conversations', event.conv_id, 'tldr_echo']):
        new_tldr = (bot.memory.get_by_path(
            ['conversations', event.conv_id, 'tldr_echo']) + 1) % 3
    else:
    ...
    if TLDR_ECHO_OPTIONS[new_tldr] == "GLOBAL":
        ...
        bot.memory.set_by_path(['conversations', event.conv_id, 'tldr_echo'],
                               new_tldr)
    else:
    ...
    message = _('<b>TLDR echo setting for this hangout has been set to {'
                '0}.</b>').format(TLDR_ECHO_OPTIONS[new_tldr])
...
async def tldr(bot, event, *args):
    ...
    if bot.memory.exists(['conversations', event.conv_id, 'tldr_echo']):
        tldr_echo = bot.memory.get_by_path(
            ['conversations', event.conv_id, 'tldr_echo'])
    else:
...
def tldr_base(bot, conv_id, parameters):
    ...
    # If it is, then add the rest of the rooms to the list of conversations to
    #  process
    if syncouts:
    ...
    if parameters[0] == "clear":
        if len(parameters) == 2 and parameters[1].isdigit():
            ...
            if key_index < 0 or key_index >= len(sorted_keys):
                ...
                bot.memory.save()
                message = _('TL;DR #{} removed - "{}"').format(parameters[1],
                                                               popped_tldr)
        ...
        if len(parameters) > 2 and parameters[1].isdigit():
            ...
            if key_index < 0 or key_index >= len(sorted_keys):
                ...
                for conv in conv_id_list:
                    bot.memory.set_by_path(['tldr', conv], conv_tldr)
                bot.memory.save()
                message = _('TL;DR #{} edited - "{}" -> "{}"').format(
                    parameters[1], edited_tldr, text)
        else:
        ...
        if text:
            ...
            for conv in conv_id_list:
                bot.memory.set_by_path(['tldr', conv], conv_tldr)
            bot.memory.save()
            message = _('<em>{text}</em> added to TL;DR. Count: {num}').format(
                text=text, num=len(conv_tldr))
